...
type Block struct {
	...
	Height          int64   `xorm:"height int notnull unique(height)"`
	...
	Hash            string  `xorm:"hash char(64) notnull index(hash)"`
	...
	PoolName        string  `xorm:"pool_name varchar(100) notnull default '' index(pool_name)"`
}
